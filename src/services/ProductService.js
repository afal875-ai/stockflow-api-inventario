...
class ProductService {
  ...
  async create(payload) {
    ...
    await this.inventoryRepository.createProduct(product);
    return this.decorate(product);
  }
  ...
  async update(id, payload) {
    ...
    if (!current) {
      throw new AppError("El producto que intenta editar no existe.", 404, null, "PRODUCT_NOT_FOUND");
    }
  ...
  async delete(id) {
    const deleted = await this.inventoryRepository.deleteProduct(id);

    if (!deleted) {
      throw new AppError("El producto que intenta eliminar no existe.", 404, null, "PRODUCT_NOT_FOUND");
    }
  }
  ...
  async getCategories() {
    const products = await this.inventoryRepository.findAllProducts();
    return [...new Set(products.map((product) => product.category))].sort((first, second) =>
  ...
  validate(payload) {
    ...
    if (errors.length > 0) {
      throw new AppError("Revise la información ingresada.", 422, errors, "VALIDATION_ERROR");
    }
  ...
  async ensureUniqueSku(sku, excludedId = null) {
    ...
    if (duplicate) {
      throw new AppError("Ya existe un producto registrado con ese SKU.", 409, null, "DUPLICATE_SKU");
    }
  }
